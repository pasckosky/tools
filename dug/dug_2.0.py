...
def show_help(x):
    ...
    p("")
    p("""Special options:
    --get_new:    Get lastest version of dug
...
def download_last(ref_version):
    fn_get = request_http()
    lastest_url = "https://dl.dropboxusercontent.com/u/1412272/Dug/lastest"
    lastest_version = fn_get(lastest_url).strip()
    if lastest_version == ref_version:
    ...
    if script_file == "":
        ...
        sys.exit(1)
    fout = open("/tmp/dug.py","w")
    utf8 = codecs.getwriter('utf8')
    utf8(fout).write(script_file)
    fout.close()
    print "File version %s has been saved as /tmp/dug.py\nMove it wherever you please"%lastest_version
    sys.exit(0)
